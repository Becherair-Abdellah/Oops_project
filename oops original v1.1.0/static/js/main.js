...
function createPost(imguser, nameUser, timeCrtPost, textPost, imgPost, number_up, number_down, number_comments, commentsPost, iconPublic,idPOst) {
...
function add_Post() {
    ...
    btnsend.onclick = () => {
        ...
        IsImage = false;
        Istext = false;
        createPost("../static/imgs/imgcode/linkedin.jpg", 'Abdellah Becherair', '15 oct 2023', `${textarea.value}`
            , paths_img, 200, 10, 4, '', ICONpRIVATE);
        addpost.remove();
        document.querySelector('#add-post').children[0].style = '0deg';
        paths_img = [];
        document.querySelector('.text-h2') ? document.querySelector('.text-h2').remove() : '';
        postServer(textarea.value,false);
    }
...
async function postServer(text,priPub = true){
    console.log('peoplw');
...
async function getPosts(){
   ...
   let data = await response.json();
    ...
    for(let post of data.posts){
        // console.log(post);
        createPost("../static/imgs/imgcode/linkedin.jpg", 'Abdellah Becherair', '15 oct 2023', `${post.content}`
            , paths_img, 200, 10, 4, '',default_img,post.id);
    }
}
getPosts();
controlTop();
