...
let child ;
    ...
    for (i = 0; i < 4; i++) {
        nbrComments++;
        createComments(allComments, 'abdellah becherair and tools foods main',nbrComments,child);
    }
    console.log(nbrComments);
    // create div 'veiw comments ' ;
    let nbrcomments
    if (nbrComments > 1) {
        child = allComments.children[0];
        comments.prepend(child);
        comments.prepend(h2Comments);
...
// function Comments
function createComments(parent, text,nbr,child) {
    // img commenter
    ...
    dots_Comments.onclick = (e) => {
        ...
        dotsComments(name_comment, dots_Comments, intract_comment,parent.parentNode,nbr,child);
        // disable clicking
        setTimeout(() => {
            document.onclick = () => {
                document.querySelector('.fun-comment') ? document.querySelector('.fun-comment').remove() : '';
...
function dotsComments(parent, eleMain, elechange,BIGPARENT,nbr,child) {
    console.log(show);
    ...
    li_delet.onclick = ()=>{
        ...
        if(show){
            check_nbr_comments(BIGPARENT.children[3],BIGPARENT.children[2],nbr,BIGPARENT,child);
        }else{
        check_nbr_comments(BIGPARENT.children[2],BIGPARENT.children[1],nbr,BIGPARENT,child);
        }
        }
...
// function to check of number of comments
function check_nbr_comments(divShow,allComments,nbr,divComments,child){
    console.log(nbr);
    if(nbr==0){
        setTimeout(()=>{
            divShow.innerText = 'No comments here';
            divShow.style = 'margin:0 auto';
        },500);
    ...
    if(show && nbr>0){
        console.log(allComments.children[0]);

        setTimeout(()=>{
        divComments.insertBefore(allComments.children[0],divComments.children[1]);
        },510)
        }
}
controlTop();
